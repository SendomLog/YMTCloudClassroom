...
- (COPromise *)login {
    ...
    return [COPromise promise:^(COPromiseFulfill  _Nonnull fullfill, COPromiseReject  _Nonnull reject) {
        ...
        [[YMTHTTPSessionManager shareManager] fetch:@"user_login" parameters:dic progress:nil result:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject, NSError * _Nonnull error) {
            if (responseObject) {
